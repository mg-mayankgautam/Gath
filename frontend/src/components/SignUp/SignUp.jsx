...
const SignUp = ({ setShowModal }) => {
    ...
    const googleLogin = useGoogleLogin({
        onSuccess: async (credentialResponse) => {
            console.log("Google login SUCCESS credentialResponse:", credentialResponse);  // ✅ Log 1
            try {
                setLoading(true);
    ...
    const handleSubmit = async (e) => {
        ...
        if (Object.keys(newErrors).length === 0) {
            setLoading(true);
            try {
                const response = await axios.post(`${process.env.REACT_APP_BACKEND_URL}/auth/signup`, formData, { withCredentials: true });
                const { accessToken } = response.data;
                const decodedToken = jwtDecode(accessToken);
                ...
                if (error.response?.data?.message) {
                    ...
                    setErrors(prev => ({ ...prev, email: "Failed to create account. Please try again." }));
                }
            } finally {
                setLoading(false);
            }
    ...
    return (
        <div className="bg-[#121212CC] h-screen fixed inset-0 z-50 modalOverflow">
            <div className="bigscreen max-h-[100vh] h-[100vh] p-10">
                <div className={`relative max-w-[1024px] mx-auto h-full max-h-full border p-8 ${
                        darkMode ? "bg-[#10130D] border-[#1E1E1E]" : "bg-white border-[#CBCBCB]"
                    ...
                    <div className="flex flex-col gap-8 py-12">
                        <div className="h-8">
                            <img src={darkMode ? logowhite : logo} className="object-contain h-full" alt="Logo" />
                        </div>
                        ...
                        <form onSubmit={handleSubmit}>
                            <div className="flex gap-4">
                                <button
                                    type="button"
                                    onClick={() => googleLogin()}
                                    className={darkMode ? "input dark w-1/2 flex items-center justify-center" : "input w-1/2 flex items-center justify-center"}
                                >
