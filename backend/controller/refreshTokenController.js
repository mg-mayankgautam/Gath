                ...
                jwt.verify(
                    ...
                    (err, decoded) => {
                        ...
                        if (err || saved.id !== decoded.username) return res.sendStatus(403);
                         console.log('saved username is same as decoded username',saved.id ,decoded.username)
                        const accessToken = jwt.sign(
                            { "username": decoded.username, "role": decoded.role,"_id":decoded._id },
        ...
        .then((saved) => {
            ...
            if (saved) {
                ...
                jwt.verify(
                    ...
                    (err, decoded) => {
                        console.log(decoded)
                        if (err || saved.email !== decoded.email) return res.sendStatus(403);
                        // console.log('saved username is same as decoded username')
                        const accessToken = jwt.sign(
                            { "username": decoded.email, "role": decoded.role,"_id":decoded._id },
                            process.env.ACCESS_TOKEN_SECRET,
                            { expiresIn: '600s' }
                        );
